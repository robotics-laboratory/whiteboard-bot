...
CameraNode::CameraNode() : Node("CameraNode") {
    ...
    calibration_params_ = import_camera_calibration(path_to_calibration_file);
...
cv::Mat CameraNode::removeDistorsion(const cv::Mat& raw_image) {
    ...
    cv::initUndistortRectifyMap(
        ...
        calibration_params_.distorsion,
        cv::Matx33f::eye(),
        calibration_params_.camera_matrix,
        frame_size_,
        CV_32FC1,
...
void CameraNode::publishPreview(const cv::Mat& warped_image) {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Preview published successfully");
}

void CameraNode::handleCameraOnTimer() {
    const cv::Mat raw_image = getImage();

    const cv::Mat undistorted_image = removeDistorsion(raw_image);
